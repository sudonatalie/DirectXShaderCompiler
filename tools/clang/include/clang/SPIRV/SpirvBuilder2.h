//===-- SpirvBuilder2.h - SPIR-V Builder -----------------------*- C++ -*---===//
//
...
//
//===----------------------------------------------------------------------===//
#ifndef LLVM_CLANG_SPIRV_SPIRVBUILDER2_H
#define LLVM_CLANG_SPIRV_SPIRVBUILDER2_H
...
// Provides StringMapInfo2 for std::string so we can create a DenseMap with key
// of type std::string.
struct StringMapInfo2 {
...
public:
  SpirvBuilder2(SpirvContext &c, const SpirvCodeGenOptions &);
  ~SpirvBuilder2() = default;

  // Forbid copy construction and assignment
  SpirvBuilder2(const SpirvBuilder2 &) = delete;
  SpirvBuilder2 &operator=(const SpirvBuilder2 &) = delete;

  // Forbid move construction and assignment
  SpirvBuilder2(SpirvBuilder2 &&) = delete;
  SpirvBuilder2 &operator=(SpirvBuilder2 &&) = delete;
...
private:
  ...
  // the SpirvBuilder2 will generate and reuse them.
  llvm::DenseMap<std::string, SpirvString *, StringMapInfo2> stringLiterals;
...
SpirvString *
SpirvBuilder2::setDebugSource(uint32_t major, uint32_t minor,
...
void SpirvBuilder2::addExecutionMode(SpirvFunction *entryPoint,
...
#endif // LLVM_CLANG_SPIRV_SPIRVBUILDER2_H
