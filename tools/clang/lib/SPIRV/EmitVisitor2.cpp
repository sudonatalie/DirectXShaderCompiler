//===--- EmitVisitor2.cpp - SPIR-V Emit Visitor Implementation ----*- C++ -*-==//
//
...
EmitVisitor2::Header::Header(uint32_t bound_, uint32_t version_)
...
template <>
uint32_t
EmitVisitor2::getOrAssignResultId<SpirvInstruction>(SpirvInstruction *obj) {
...
std::vector<uint32_t> EmitVisitor2::Header::takeBinary() {
...
uint32_t EmitVisitor2::getOrCreateOpStringId(llvm::StringRef str) {
...
uint32_t EmitVisitor2::getLiteralEncodedForDebugInfo(uint32_t val) {
...
void EmitVisitor2::emitDebugNameForInstruction(uint32_t resultId,
...
void EmitVisitor2::emitDebugLine(spv::Op op, const SourceLocation &loc,
...
void EmitVisitor2::initInstruction(SpirvInstruction *inst) {
...
void EmitVisitor2::initInstruction(spv::Op op, const SourceLocation &loc) {
...
void EmitVisitor2::finalizeInstruction(std::vector<uint32_t> *section) {
  assert(section);
  curInst[0] |= static_cast<uint32_t>(curInst.size()) << 16;
  section->insert(section->end(), curInst.begin(), curInst.end());
}

std::vector<uint32_t> EmitVisitor2::takeBinary() {
...
void EmitVisitor2::encodeString(llvm::StringRef value) {
  const auto &words = string::encodeSPIRVString(value);
  curInst.insert(curInst.end(), words.begin(), words.end());
}

bool EmitVisitor2::visit(SpirvModule *, Phase) {
  // No pre-visit operations needed for SpirvModule.
  return true;
}

bool EmitVisitor2::visit(SpirvFunction *fn, Phase phase) {
  assert(fn);
...
bool EmitVisitor2::visit(SpirvBasicBlock *bb, Phase phase) {
  assert(bb);
...
bool EmitVisitor2::visit(SpirvCapability *cap) {
  initInstruction(cap);
...
bool EmitVisitor2::visit(SpirvExtension *ext) {
  initInstruction(ext);
...
bool EmitVisitor2::visit(SpirvExtInstImport *inst) {
...
bool EmitVisitor2::visit(SpirvMemoryModel *inst) {
  initInstruction(inst);
...
bool EmitVisitor2::visit(SpirvEntryPoint *inst) {
  initInstruction(inst);
...
bool EmitVisitor2::visit(SpirvExecutionMode *inst) {
  initInstruction(inst);
  curInst.push_back(getOrAssignResultId<SpirvFunction>(inst->getEntryPoint()));
...
bool EmitVisitor2::visit(SpirvString *inst) {
...
bool EmitVisitor2::visit(SpirvSource *inst) {
...
bool EmitVisitor2::visit(SpirvModuleProcessed *inst) {
  initInstruction(inst);
...
bool EmitVisitor2::visit(SpirvDecoration *inst) {
  initInstruction(inst);
  if (inst->getTarget()) {
...
bool EmitVisitor2::visit(SpirvVariable *inst) {
  initInstruction(inst);
...
bool EmitVisitor2::visit(SpirvFunctionParameter *inst) {
  initInstruction(inst);
  curInst.push_back(inst->getResultTypeId());
  curInst.push_back(getOrAssignResultId<SpirvInstruction>(inst));
  finalizeInstruction(&mainBinary);
...
bool EmitVisitor2::visit(SpirvLoopMerge *inst) {
  initInstruction(inst);
  curInst.push_back(
...
bool EmitVisitor2::visit(SpirvSelectionMerge *inst) {
  initInstruction(inst);
  curInst.push_back(
      getOrAssignResultId<SpirvBasicBlock>(inst->getMergeBlock()));
...
bool EmitVisitor2::visit(SpirvBranch *inst) {
  initInstruction(inst);
  curInst.push_back(
...
bool EmitVisitor2::visit(SpirvBranchConditional *inst) {
  initInstruction(inst);
  curInst.push_back(
...
bool EmitVisitor2::visit(SpirvKill *inst) {
  initInstruction(inst);
  finalizeInstruction(&mainBinary);
  return true;
}

bool EmitVisitor2::visit(SpirvReturn *inst) {
  initInstruction(inst);
...
bool EmitVisitor2::visit(SpirvSwitch *inst) {
  initInstruction(inst);
...
bool EmitVisitor2::visit(SpirvUnreachable *inst) {
  initInstruction(inst);
  finalizeInstruction(&mainBinary);
  return true;
}

bool EmitVisitor2::visit(SpirvAccessChain *inst) {
  initInstruction(inst);
  curInst.push_back(inst->getResultTypeId());
  curInst.push_back(getOrAssignResultId<SpirvInstruction>(inst));
...
bool EmitVisitor2::visit(SpirvAtomic *inst) {
  const auto op = inst->getopcode();
  initInstruction(inst);
...
bool EmitVisitor2::visit(SpirvBarrier *inst) {
...
bool EmitVisitor2::visit(SpirvBinaryOp *inst) {
  initInstruction(inst);
  curInst.push_back(inst->getResultTypeId());
  curInst.push_back(getOrAssignResultId<SpirvInstruction>(inst));
...
bool EmitVisitor2::visit(SpirvBitFieldExtract *inst) {
  initInstruction(inst);
  curInst.push_back(inst->getResultTypeId());
  curInst.push_back(getOrAssignResultId<SpirvInstruction>(inst));
  curInst.push_back(getOrAssignResultId<SpirvInstruction>(inst->getBase()));
...
bool EmitVisitor2::visit(SpirvBitFieldInsert *inst) {
  initInstruction(inst);
  curInst.push_back(inst->getResultTypeId());
  curInst.push_back(getOrAssignResultId<SpirvInstruction>(inst));
  curInst.push_back(getOrAssignResultId<SpirvInstruction>(inst->getBase()));
...
bool EmitVisitor2::visit(SpirvConstantBoolean *inst) {
...
bool EmitVisitor2::visit(SpirvConstantInteger *inst) {
...
bool EmitVisitor2::visit(SpirvConstantFloat *inst) {
  typeHandler.getOrCreateConstant(inst);
  emitDebugNameForInstruction(getOrAssignResultId<SpirvInstruction>(inst),
                              inst->getDebugName());
  return true;
...
bool EmitVisitor2::visit(SpirvConstantComposite *inst) {
  typeHandler.getOrCreateConstant(inst);
  emitDebugNameForInstruction(getOrAssignResultId<SpirvInstruction>(inst),
                              inst->getDebugName());
  return true;
...
bool EmitVisitor2::visit(SpirvConstantNull *inst) {
  typeHandler.getOrCreateConstant(inst);
  emitDebugNameForInstruction(getOrAssignResultId<SpirvInstruction>(inst),
                              inst->getDebugName());
  return true;
...
bool EmitVisitor2::visit(SpirvCompositeConstruct *inst) {
  initInstruction(inst);
  curInst.push_back(inst->getResultTypeId());
  curInst.push_back(getOrAssignResultId<SpirvInstruction>(inst));
...
bool EmitVisitor2::visit(SpirvCompositeExtract *inst) {
  initInstruction(inst);
  curInst.push_back(inst->getResultTypeId());
  curInst.push_back(getOrAssignResultId<SpirvInstruction>(inst));
  curInst.push_back(
...
bool EmitVisitor2::visit(SpirvCompositeInsert *inst) {
  initInstruction(inst);
  curInst.push_back(inst->getResultTypeId());
  curInst.push_back(getOrAssignResultId<SpirvInstruction>(inst));
...
bool EmitVisitor2::visit(SpirvEmitVertex *inst) {
  initInstruction(inst);
  finalizeInstruction(&mainBinary);
  return true;
}

bool EmitVisitor2::visit(SpirvEndPrimitive *inst) {
  initInstruction(inst);
  finalizeInstruction(&mainBinary);
  return true;
}

bool EmitVisitor2::visit(SpirvExtInst *inst) {
  initInstruction(inst);
  curInst.push_back(inst->getResultTypeId());
  curInst.push_back(getOrAssignResultId<SpirvInstruction>(inst));
  curInst.push_back(
...
bool EmitVisitor2::visit(SpirvFunctionCall *inst) {
  initInstruction(inst);
  curInst.push_back(inst->getResultTypeId());
  curInst.push_back(getOrAssignResultId<SpirvInstruction>(inst));
...
bool EmitVisitor2::visit(SpirvNonUniformBinaryOp *inst) {
  initInstruction(inst);
  curInst.push_back(inst->getResultTypeId());
  curInst.push_back(getOrAssignResultId<SpirvInstruction>(inst));
  curInst.push_back(typeHandler.getOrCreateConstantInt(
...
bool EmitVisitor2::visit(SpirvNonUniformElect *inst) {
  initInstruction(inst);
  curInst.push_back(inst->getResultTypeId());
  curInst.push_back(getOrAssignResultId<SpirvInstruction>(inst));
  curInst.push_back(typeHandler.getOrCreateConstantInt(
...
bool EmitVisitor2::visit(SpirvNonUniformUnaryOp *inst) {
  initInstruction(inst);
  curInst.push_back(inst->getResultTypeId());
  curInst.push_back(getOrAssignResultId<SpirvInstruction>(inst));
  curInst.push_back(typeHandler.getOrCreateConstantInt(
...
bool EmitVisitor2::visit(SpirvImageOp *inst) {
  initInstruction(inst);
...
bool EmitVisitor2::visit(SpirvImageQuery *inst) {
  initInstruction(inst);
  curInst.push_back(inst->getResultTypeId());
  curInst.push_back(getOrAssignResultId<SpirvInstruction>(inst));
  curInst.push_back(getOrAssignResultId<SpirvInstruction>(inst->getImage()));
...
bool EmitVisitor2::visit(SpirvImageSparseTexelsResident *inst) {
  initInstruction(inst);
  curInst.push_back(inst->getResultTypeId());
  curInst.push_back(getOrAssignResultId<SpirvInstruction>(inst));
  curInst.push_back(
...
bool EmitVisitor2::visit(SpirvImageTexelPointer *inst) {
  initInstruction(inst);
  curInst.push_back(inst->getResultTypeId());
  curInst.push_back(getOrAssignResultId<SpirvInstruction>(inst));
  curInst.push_back(getOrAssignResultId<SpirvInstruction>(inst->getImage()));
...
bool EmitVisitor2::visit(SpirvLoad *inst) {
  initInstruction(inst);
  curInst.push_back(inst->getResultTypeId());
  curInst.push_back(getOrAssignResultId<SpirvInstruction>(inst));
  curInst.push_back(getOrAssignResultId<SpirvInstruction>(inst->getPointer()));
...
bool EmitVisitor2::visit(SpirvCopyObject *inst) {
  initInstruction(inst);
  curInst.push_back(inst->getResultTypeId());
  curInst.push_back(getOrAssignResultId<SpirvInstruction>(inst));
  curInst.push_back(getOrAssignResultId<SpirvInstruction>(inst->getPointer()));
...
bool EmitVisitor2::visit(SpirvSampledImage *inst) {
  initInstruction(inst);
  curInst.push_back(inst->getResultTypeId());
  curInst.push_back(getOrAssignResultId<SpirvInstruction>(inst));
  curInst.push_back(getOrAssignResultId<SpirvInstruction>(inst->getImage()));
...
bool EmitVisitor2::visit(SpirvSelect *inst) {
  initInstruction(inst);
  curInst.push_back(inst->getResultTypeId());
  curInst.push_back(getOrAssignResultId<SpirvInstruction>(inst));
  curInst.push_back(
...
bool EmitVisitor2::visit(SpirvSpecConstantBinaryOp *inst) {
  initInstruction(inst);
  curInst.push_back(inst->getResultTypeId());
  curInst.push_back(getOrAssignResultId<SpirvInstruction>(inst));
...
bool EmitVisitor2::visit(SpirvSpecConstantUnaryOp *inst) {
  initInstruction(inst);
  curInst.push_back(inst->getResultTypeId());
  curInst.push_back(getOrAssignResultId<SpirvInstruction>(inst));
  curInst.push_back(static_cast<uint32_t>(inst->getSpecConstantopcode()));
...
bool EmitVisitor2::visit(SpirvStore *inst) {
  initInstruction(inst);
  curInst.push_back(getOrAssignResultId<SpirvInstruction>(inst->getPointer()));
  curInst.push_back(getOrAssignResultId<SpirvInstruction>(inst->getObject()));
  if (inst->hasMemoryAccessSemantics())
...
bool EmitVisitor2::visit(SpirvUnaryOp *inst) {
  initInstruction(inst);
  curInst.push_back(inst->getResultTypeId());
  curInst.push_back(getOrAssignResultId<SpirvInstruction>(inst));
  curInst.push_back(getOrAssignResultId<SpirvInstruction>(inst->getOperand()));
...
bool EmitVisitor2::visit(SpirvVectorShuffle *inst) {
  initInstruction(inst);
  curInst.push_back(inst->getResultTypeId());
  curInst.push_back(getOrAssignResultId<SpirvInstruction>(inst));
...
bool EmitVisitor2::visit(SpirvArrayLength *inst) {
  initInstruction(inst);
  curInst.push_back(inst->getResultTypeId());
  curInst.push_back(getOrAssignResultId<SpirvInstruction>(inst));
  curInst.push_back(
...
bool EmitVisitor2::visit(SpirvRayTracingOpNV *inst) {
  initInstruction(inst);
  if (inst->hasResultType()) {
    curInst.push_back(inst->getResultTypeId());
    curInst.push_back(getOrAssignResultId<SpirvInstruction>(inst));
...
bool EmitVisitor2::visit(SpirvDemoteToHelperInvocationEXT *inst) {
  initInstruction(inst);
  finalizeInstruction(&mainBinary);
  return true;
}

bool EmitVisitor2::visit(SpirvDebugInfoNone *inst) {
  initInstruction(inst);
  curInst.push_back(inst->getResultTypeId());
  curInst.push_back(getOrAssignResultId<SpirvInstruction>(inst));
  curInst.push_back(
...
void EmitVisitor2::generateDebugSource(uint32_t fileId, uint32_t textId,
...
void EmitVisitor2::generateDebugSourceContinued(uint32_t textId,
                                               SpirvDebugSource *inst) {
...
void EmitVisitor2::generateChoppedSource(uint32_t fileId, SpirvDebugSource *inst) {
  // Chop up the source into multiple segments if it is too long.
  llvm::Optional<llvm::StringRef> firstSnippet = llvm::None;
  llvm::SmallVector<llvm::StringRef, 2> choppedSrcCode;
  std::string text;
...
bool EmitVisitor2::visit(SpirvDebugSource *inst) {
  // Emit the OpString for the file name.
...
bool EmitVisitor2::visit(SpirvDebugCompilationUnit *inst) {
  initInstruction(inst);
  curInst.push_back(inst->getResultTypeId());
  curInst.push_back(getOrAssignResultId<SpirvInstruction>(inst));
  curInst.push_back(
...
bool EmitVisitor2::visit(SpirvDebugLexicalBlock *inst) {
  initInstruction(inst);
  curInst.push_back(inst->getResultTypeId());
  curInst.push_back(getOrAssignResultId<SpirvInstruction>(inst));
  curInst.push_back(
...
bool EmitVisitor2::visit(SpirvDebugScope *inst) {
...
bool EmitVisitor2::visit(SpirvDebugFunctionDeclaration *inst) {
...
bool EmitVisitor2::visit(SpirvDebugFunction *inst) {
  uint32_t nameId = getOrCreateOpStringId(inst->getDebugName());
  uint32_t linkageNameId = getOrCreateOpStringId(inst->getLinkageName());
  initInstruction(inst);
  curInst.push_back(inst->getResultTypeId());
...
bool EmitVisitor2::visit(SpirvDebugFunctionDefinition *inst) {
  initInstruction(inst);
  curInst.push_back(inst->getResultTypeId());
  curInst.push_back(getOrAssignResultId<SpirvInstruction>(inst));
  curInst.push_back(
...
bool EmitVisitor2::visit(SpirvDebugTypeBasic *inst) {
...
bool EmitVisitor2::visit(SpirvDebugTypeVector *inst) {
  initInstruction(inst);
  curInst.push_back(inst->getResultTypeId());
  curInst.push_back(getOrAssignResultId<SpirvInstruction>(inst));
  curInst.push_back(
...
bool EmitVisitor2::visit(SpirvDebugTypeArray *inst) {
  initInstruction(inst);
  curInst.push_back(inst->getResultTypeId());
  curInst.push_back(getOrAssignResultId<SpirvInstruction>(inst));
  curInst.push_back(
...
bool EmitVisitor2::visit(SpirvDebugTypeFunction *inst) {
  initInstruction(inst);
  curInst.push_back(inst->getResultTypeId());
  curInst.push_back(getOrAssignResultId<SpirvInstruction>(inst));
  curInst.push_back(
...
bool EmitVisitor2::visit(SpirvDebugTypeComposite *inst) {
  uint32_t typeNameId = getOrCreateOpStringId(inst->getDebugName());
  uint32_t linkageNameId = getOrCreateOpStringId(inst->getLinkageName());
...
bool EmitVisitor2::visit(SpirvDebugTypeMember *inst) {
  uint32_t typeNameId = getOrCreateOpStringId(inst->getDebugName());
...
bool EmitVisitor2::visit(SpirvDebugTypeTemplate *inst) {
  initInstruction(inst);
  curInst.push_back(inst->getResultTypeId());
  curInst.push_back(getOrAssignResultId<SpirvInstruction>(inst));
  curInst.push_back(
...
bool EmitVisitor2::visit(SpirvDebugTypeTemplateParameter *inst) {
  uint32_t typeNameId = getOrCreateOpStringId(inst->getDebugName());
  initInstruction(inst);
  curInst.push_back(inst->getResultTypeId());
  curInst.push_back(getOrAssignResultId<SpirvInstruction>(inst));
...
bool EmitVisitor2::visit(SpirvDebugLocalVariable *inst) {
  uint32_t nameId = getOrCreateOpStringId(inst->getDebugName());
  initInstruction(inst);
  curInst.push_back(inst->getResultTypeId());
  curInst.push_back(getOrAssignResultId<SpirvInstruction>(inst));
...
bool EmitVisitor2::visit(SpirvDebugDeclare *inst) {
  initInstruction(inst);
  curInst.push_back(inst->getResultTypeId());
  curInst.push_back(getOrAssignResultId<SpirvInstruction>(inst));
  curInst.push_back(
...
bool EmitVisitor2::visit(SpirvDebugGlobalVariable *inst) {
  uint32_t nameId = getOrCreateOpStringId(inst->getDebugName());
  uint32_t linkageNameId = getOrCreateOpStringId(inst->getLinkageName());
  initInstruction(inst);
  curInst.push_back(inst->getResultTypeId());
...
bool EmitVisitor2::visit(SpirvDebugExpression *inst) {
  initInstruction(inst);
  curInst.push_back(inst->getResultTypeId());
  curInst.push_back(getOrAssignResultId<SpirvInstruction>(inst));
  curInst.push_back(
...
bool EmitVisitor2::visit(SpirvRayQueryOpKHR *inst) {
  initInstruction(inst);
  if (inst->hasResultType()) {
    curInst.push_back(inst->getResultTypeId());
    curInst.push_back(getOrAssignResultId<SpirvInstruction>(inst));
...
bool EmitVisitor2::visit(SpirvReadClock *inst) {
  initInstruction(inst);
  curInst.push_back(inst->getResultTypeId());
  curInst.push_back(getOrAssignResultId<SpirvInstruction>(inst));
  curInst.push_back(getOrAssignResultId<SpirvInstruction>(inst->getScope()));
...
bool EmitVisitor2::visit(SpirvRayTracingTerminateOpKHR *inst) {
  initInstruction(inst);
  finalizeInstruction(&mainBinary);
  return true;
}

bool EmitVisitor2::visit(SpirvIntrinsicInstruction *inst) {
  initInstruction(inst);
  if (inst->hasResultType()) {
    curInst.push_back(inst->getResultTypeId());
    curInst.push_back(getOrAssignResultId<SpirvInstruction>(inst));
...
uint32_t EmitTypeHandler2::getOrCreateConstant(SpirvConstant *inst) {
...
uint32_t EmitTypeHandler2::getOrCreateConstantBool(SpirvConstantBoolean *inst) {
...
uint32_t EmitTypeHandler2::getOrCreateConstantNull(SpirvConstantNull *inst) {
  auto found =
...
uint32_t EmitTypeHandler2::getOrCreateConstantFloat(SpirvConstantFloat *inst) {
...
uint32_t
EmitTypeHandler2::getOrCreateConstantInt(llvm::APInt value,
...
uint32_t
EmitTypeHandler2::getOrCreateConstantComposite(SpirvConstantComposite *inst) {
...
uint32_t EmitTypeHandler2::emitType(const SpirvType *type) {
  ...
  // Structure types
    ...
    if (interfaceType == StructInterfaceType::StorageBuffer)
      ...
      emitDecoration(id,
                     true //featureManager.isTargetEnvVulkan1p2OrAbove()
...
void EmitTypeHandler2::emitDecoration(uint32_t typeResultId,
...
void EmitTypeHandler2::emitNameForType(llvm::StringRef name,
