//===--- SpirvBuilder2.cpp - SPIR-V Builder Implementation --------*- C++ -*-==//
//
...
SpirvBuilder2::SpirvBuilder2(SpirvContext &ctx,
                           const SpirvCodeGenOptions &opt)
    : context(ctx), mod(llvm::make_unique<SpirvModule>()),
...
SpirvFunction *SpirvBuilder2::createSpirvFunction(QualType returnType,
                                                 SourceLocation loc,
                                                 llvm::StringRef name,
                                                 bool isPrecise,
                                                 bool isNoInline) {
...
SpirvFunction *SpirvBuilder2::beginFunction(QualType returnType,
                                           SourceLocation loc,
...
SpirvFunctionParameter *SpirvBuilder2::addFnParam(QualType ptrType,
                                                 bool isPrecise,
                                                 SourceLocation loc,
                                                 llvm::StringRef name) {
...
SpirvVariable *SpirvBuilder2::addFnVar(QualType valueType, SourceLocation loc,
...
void SpirvBuilder2::endFunction() {
...
SpirvBasicBlock *SpirvBuilder2::createBasicBlock(llvm::StringRef name) {
...
SpirvDebugScope *SpirvBuilder2::createDebugScope(SpirvDebugInstruction *scope) {
...
void SpirvBuilder2::addSuccessor(SpirvBasicBlock *successorBB) {
  assert(insertPoint && "null insert point");
  insertPoint->addSuccessor(successorBB);
}

void SpirvBuilder2::setMergeTarget(SpirvBasicBlock *mergeLabel) {
  assert(insertPoint && "null insert point");
  insertPoint->setMergeTarget(mergeLabel);
}

void SpirvBuilder2::setContinueTarget(SpirvBasicBlock *continueLabel) {
  assert(insertPoint && "null insert point");
  insertPoint->setContinueTarget(continueLabel);
}

SpirvCompositeConstruct *SpirvBuilder2::createCompositeConstruct(
...
SpirvCompositeExtract *SpirvBuilder2::createCompositeExtract(
...
SpirvCompositeInsert *SpirvBuilder2::createCompositeInsert(
    QualType resultType, SpirvInstruction *composite,
...
SpirvVectorShuffle *SpirvBuilder2::createVectorShuffle(
...
SpirvLoad *SpirvBuilder2::createLoad(QualType resultType,
...
SpirvCopyObject *SpirvBuilder2::createCopyObject(QualType resultType,
                                                SpirvInstruction *pointer,
                                                SourceLocation loc) {
  assert(insertPoint && "null insert point");
...
SpirvLoad *SpirvBuilder2::createLoad(const SpirvType *resultType,
                                    SpirvInstruction *pointer,
                                    SourceLocation loc) {
  assert(insertPoint && "null insert point");
...
void SpirvBuilder2::createStore(SpirvInstruction *address,
...
SpirvFunctionCall *
SpirvBuilder2::createFunctionCall(QualType returnType, SpirvFunction *func,
...
SpirvAccessChain *SpirvBuilder2::createAccessChain(
...
SpirvAccessChain *
SpirvBuilder2::createAccessChain(QualType resultType, SpirvInstruction *base,
...
SpirvUnaryOp *SpirvBuilder2::createUnaryOp(spv::Op op, QualType resultType,
...
SpirvBinaryOp *SpirvBuilder2::createBinaryOp(spv::Op op, QualType resultType,
...
SpirvSpecConstantBinaryOp *SpirvBuilder2::createSpecConstantBinaryOp(
...
SpirvNonUniformElect *SpirvBuilder2::createGroupNonUniformElect(
...
SpirvNonUniformUnaryOp *SpirvBuilder2::createGroupNonUniformUnaryOp(
...
SpirvNonUniformBinaryOp *SpirvBuilder2::createGroupNonUniformBinaryOp(
...
SpirvAtomic *SpirvBuilder2::createAtomicOp(
...
SpirvAtomic *SpirvBuilder2::createAtomicCompareExchange(
...
SpirvSampledImage *SpirvBuilder2::createSampledImage(QualType imageType,
...
SpirvImageTexelPointer *SpirvBuilder2::createImageTexelPointer(
...
spv::ImageOperandsMask SpirvBuilder2::composeImageOperandsMask(
...
/*
SpirvInstruction *SpirvBuilder2::createImageSample(
...
//SpirvInstruction *SpirvBuilder2::createImageFetchOrRead(
...
//  }
//
//void SpirvBuilder2::createImageWrite(QualType imageType, SpirvInstruction *image,
...
//}
//
//SpirvInstruction *SpirvBuilder2::createImageGather(
//    QualType texelType, QualType imageType, SpirvInstruction *image,
//    SpirvInstruction *sampler, SpirvInstruction *coordinate,
...
//SpirvBuilder2::createImageSparseTexelsResident(SpirvInstruction *residentCode,
//                                              SourceLocation loc) {
//  assert(insertPoint && "null insert point");
...
//}
//
//SpirvImageQuery *SpirvBuilder2::createImageQuery(spv::Op opcode,
//                                                QualType resultType,
//                                                SourceLocation loc,
//                                                SpirvInstruction *image,
...
SpirvSelect *SpirvBuilder2::createSelect(QualType resultType,
...
void SpirvBuilder2::createSwitch(
...
void SpirvBuilder2::createKill(SourceLocation loc) {
  assert(insertPoint && "null insert point");
  auto *kill = new (context) SpirvKill(loc);
  insertPoint->addInstruction(kill);
}

void SpirvBuilder2::createBranch(SpirvBasicBlock *targetLabel,
...
void SpirvBuilder2::createConditionalBranch(
...
void SpirvBuilder2::createReturn(SourceLocation loc) {
  assert(insertPoint && "null insert point");
  insertPoint->addInstruction(new (context) SpirvReturn(loc));
}

void SpirvBuilder2::createReturnValue(SpirvInstruction *value,
                                     SourceLocation loc) {
  assert(insertPoint && "null insert point");
...
SpirvInstruction *
SpirvBuilder2::createGLSLExtInst(QualType resultType, GLSLstd450 inst,
...
SpirvInstruction *
SpirvBuilder2::createGLSLExtInst(const SpirvType *resultType, GLSLstd450 inst,
                                llvm::ArrayRef<SpirvInstruction *> operands,
                                SourceLocation loc) {
  assert(insertPoint && "null insert point");
  auto *extInst = new (context) SpirvExtInst(
...
SpirvInstruction *SpirvBuilder2::createNonSemanticDebugPrintfExtInst(
...
void SpirvBuilder2::createBarrier(spv::Scope memoryScope,
...
SpirvBitFieldInsert *SpirvBuilder2::createBitFieldInsert(
...
SpirvBitFieldExtract *SpirvBuilder2::createBitFieldExtract(
...
void SpirvBuilder2::createEmitVertex(SourceLocation loc) {
  assert(insertPoint && "null insert point");
  auto *inst = new (context) SpirvEmitVertex(loc);
  insertPoint->addInstruction(inst);
}

void SpirvBuilder2::createEndPrimitive(SourceLocation loc) {
  assert(insertPoint && "null insert point");
  auto *inst = new (context) SpirvEndPrimitive(loc);
  insertPoint->addInstruction(inst);
}

SpirvArrayLength *SpirvBuilder2::createArrayLength(QualType resultType,
                                                  SourceLocation loc,
...
SpirvInstruction *
SpirvBuilder2::createRayTracingOpsNV(spv::Op opcode, QualType resultType,
...
SpirvInstruction *
SpirvBuilder2::createDemoteToHelperInvocationEXT(SourceLocation loc) {
  assert(insertPoint && "null insert point");
...
SpirvDebugSource *SpirvBuilder2::createDebugSource(llvm::StringRef file,
                                                  llvm::StringRef text) {
...
SpirvBuilder2::createDebugCompilationUnit(SpirvDebugSource *source) {
...
SpirvBuilder2::createDebugLexicalBlock(SpirvDebugSource *source, uint32_t line,
                                      uint32_t column,
...
SpirvDebugLocalVariable *SpirvBuilder2::createDebugLocalVariable(
...
SpirvDebugGlobalVariable *SpirvBuilder2::createDebugGlobalVariable(
...
SpirvDebugInfoNone *SpirvBuilder2::getOrCreateDebugInfoNone() {
  if (debugNone)
    return debugNone;
...
SpirvDebugExpression *SpirvBuilder2::getOrCreateNullDebugExpression() {
  if (nullDebugExpr)
...
SpirvDebugDeclare *SpirvBuilder2::createDebugDeclare(
...
SpirvDebugFunction *SpirvBuilder2::createDebugFunction(
...
SpirvBuilder2::createDebugFunctionDef(SpirvDebugFunction *function,
                                     SpirvFunction *fn) {
...
SpirvInstruction *
SpirvBuilder2::createRayQueryOpsKHR(spv::Op opcode, QualType resultType,
                                   ArrayRef<SpirvInstruction *> operands,
...
//SpirvInstruction *SpirvBuilder2::createReadClock(SpirvInstruction *scope,
//                                                SourceLocation loc) {
//  assert(insertPoint && "null insert point");
...
SpirvInstruction *SpirvBuilder2::createSpirvIntrInstExt(
...
void SpirvBuilder2::createRaytracingTerminateKHR(spv::Op opcode,
                                                SourceLocation loc) {
  assert(insertPoint && "null insert point");
...
//void SpirvBuilder2::createCopyArrayInFxcCTBufferToClone(
...
//void SpirvBuilder2::createCopyStructInFxcCTBufferToClone(
...
//}
//
//void SpirvBuilder2::createCopyInstructionsFromFxcCTBufferToClone(
...
//void SpirvBuilder2::switchInsertPointToModuleInit() {
...
SpirvVariable *SpirvBuilder2::createCloneVarForFxcCTBuffer(
...
//SpirvInstruction *
//SpirvBuilder2::initializeCloneVarForFxcCTBuffer(SpirvInstruction *instr) {
//  assert(instr);
//  if (instr == nullptr)
//    return nullptr;
...
void SpirvBuilder2::addModuleProcessed(llvm::StringRef process) {
  mod->addModuleProcessed(new (context) SpirvModuleProcessed({}, process));
}

SpirvExtInstImport *SpirvBuilder2::getExtInstSet(llvm::StringRef extName) {
...
SpirvExtInstImport *SpirvBuilder2::getDebugInfoExtInstSet(bool vulkanDebugInfo) {
  return getExtInstSet(vulkanDebugInfo ? "NonSemantic.Shader.DebugInfo.100"
                                       : "OpenCL.DebugInfo.100");
}

SpirvVariable *SpirvBuilder2::addStageIOVar(QualType type,
...
SpirvVariable *SpirvBuilder2::addStageBuiltinVar(QualType type,
                                                spv::StorageClass storageClass,
                                                spv::BuiltIn builtin,
                                                bool isPrecise,
                                                SourceLocation loc) {
...
SpirvVariable *
SpirvBuilder2::addModuleVar(QualType type, spv::StorageClass storageClass,
...
SpirvVariable *SpirvBuilder2::addModuleVar(
...
void SpirvBuilder2::decorateLocation(SpirvInstruction *target,
                                    uint32_t location) {
  auto *decor =
...
void SpirvBuilder2::decorateIndex(SpirvInstruction *target, uint32_t index,
...
void SpirvBuilder2::decorateDSetBinding(SpirvVariable *target,
                                       uint32_t setNumber,
...
void SpirvBuilder2::decorateSpecId(SpirvInstruction *target, uint32_t specId,
                                  SourceLocation srcLoc) {
  auto *decor = new (context)
...
void SpirvBuilder2::decorateInputAttachmentIndex(SpirvInstruction *target,
...
void SpirvBuilder2::decorateCounterBuffer(SpirvInstruction *mainBuffer,
...
void SpirvBuilder2::decorateHlslSemantic(SpirvInstruction *target,
...
void SpirvBuilder2::decorateCentroid(SpirvInstruction *target,
                                    SourceLocation srcLoc) {
  auto *decor =
...
void SpirvBuilder2::decorateFlat(SpirvInstruction *target,
                                SourceLocation srcLoc) {
  auto *decor =
...
void SpirvBuilder2::decorateNoPerspective(SpirvInstruction *target,
                                         SourceLocation srcLoc) {
  auto *decor = new (context)
...
void SpirvBuilder2::decorateSample(SpirvInstruction *target,
                                  SourceLocation srcLoc) {
  auto *decor =
...
void SpirvBuilder2::decoratePatch(SpirvInstruction *target,
                                 SourceLocation srcLoc) {
  auto *decor =
...
void SpirvBuilder2::decorateNoContraction(SpirvInstruction *target,
                                         SourceLocation srcLoc) {
  auto *decor = new (context)
...
void SpirvBuilder2::decoratePerPrimitiveNV(SpirvInstruction *target,
                                          SourceLocation srcLoc) {
  auto *decor = new (context)
...
void SpirvBuilder2::decoratePerTaskNV(SpirvInstruction *target, uint32_t offset,
                                     SourceLocation srcLoc) {
  auto *decor =
...
void SpirvBuilder2::decorateCoherent(SpirvInstruction *target,
                                    SourceLocation srcLoc) {
  auto *decor =
...
void SpirvBuilder2::decorateLinkage(SpirvInstruction *targetInst,
...
void SpirvBuilder2::decorateLiterals(SpirvInstruction *targetInst,
...
void SpirvBuilder2::decorateString(SpirvInstruction *target, unsigned decorate,
...
SpirvConstant *SpirvBuilder2::getConstantInt(QualType type, llvm::APInt value,
                                            bool specConst) {
...
SpirvConstant *SpirvBuilder2::getConstantFloat(QualType type,
                                              llvm::APFloat value,
                                              bool specConst) {
...
//SpirvConstant *SpirvBuilder2::getConstantBool(bool value, bool specConst) {
...
SpirvConstant *
SpirvBuilder2::getConstantComposite(QualType compositeType,
...
SpirvConstant *SpirvBuilder2::getConstantNull(QualType type) {
  // We do not care about making unique constants at this point.
...
SpirvString *SpirvBuilder2::getString(llvm::StringRef str) {
...
//void SpirvBuilder2::addModuleInitCallToEntryPoints() {
...
//}
//
//void SpirvBuilder2::endModuleInitFunction() {
//  if (moduleInitInsertPoint == nullptr ||
...
std::vector<uint32_t> SpirvBuilder2::takeModule() {
  ...
  /*RemoveBufferBlockVisitor removeBufferBlockVisitor(astContext, context,
                                                    spirvOptions);*/
  EmitVisitor2 emitVisitor(context, spirvOptions);
